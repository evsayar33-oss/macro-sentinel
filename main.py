...
if os.path.exists("cms_history.csv"):
    ...
    if not df.empty:
        ...
        # Dinamik Altın ve Emtia Sinyalleri
        a_notu = "Önerilmez (Faiz Baskısı)" if rr > 0.8 else "Güçlü Koruyucu"
        
        if oil_trend > 0.05:
            emtia_notu = "🔥 Enerji/Petrol Al"
        elif oil_trend < -0.05:
            emtia_notu = "⚠️ Zayıf (Uzak Dur)"
        else:
            emtia_notu = "⚖️ Nötr (Seçici Ol)"
        ...
        with p3:
            # YENİ: Altın kötüyse başlığı değiştir, kafa karıştırma!
            csh_label = "💵 Koruma (Nakit/Altın)" if rr <= 0.8 else "💵 Koruma (Sadece Nakit/Repo)"
            st.markdown(f"**{csh_label}: %{csh_w}**")
