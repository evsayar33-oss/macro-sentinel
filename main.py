...
class MacroSentinel:
    ...
    def fetch_fred(self, s_id, limit=100):
        try:
    ...
    def run(self):
        ...
        for s in series:
            ...
            if vals is not None and not vals.empty:
                data[s] = vals.iloc[0]
                if s == 'NAPM': data['pmi_avg'] = vals.head(3).mean() # 3 aylık ortalama
            else:
                data[s] = 0.0 # Hata durumunda nötrle
        ...
        # CMS Hesaplama (Plandaki Ağırlıklarla)
        # NDL Z-Skoru simülasyonu (Zenginleştirilmiş)
