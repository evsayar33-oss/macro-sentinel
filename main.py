...
class SovereignMacroEngine:
    ...
    def run(self):
        ...
        # Döviz Kurları ve Fiyatlar
        y_data = yf.download(["HG=F", "GC=F", "ES=F", "EURUSD=X", "JPYUSD=X"], period="2y", progress=False)['Close'].ffill()
        
        # 2. GLOBAL LİKİDİTE KALİBRASYONU
        try:
            cur_eur = y_data['EURUSD=X'].iloc[-1]
            cur_jpy = y_data['JPYUSD=X'].iloc[-1]
            
            fed_usd = raw['fed'].iloc[0] # Millions
            ecb_usd = raw['ecb'].iloc[0] * cur_eur # Millions
            # BoJ verisi 100 Milyon Yen gelir. USD'ye çevir: (Değer * 100) * JPYUSD
            boj_usd = (raw['boj'].iloc[0] * 100) * cur_jpy 
            
            g3_liq = fed_usd + ecb_usd + boj_usd
            ndl = fed_usd - raw['tga'].iloc[0] - (raw['rrp'].iloc[0] * 1000)
        except: g3_liq, ndl = 22000000, 6800000
        ...
        corrs = factors.tail(60).corrwith(spx_ret.tail(60)).abs().fillna(0)
        ...
        copper_gold = y_data['HG=F'].iloc[-1] / y_data['GC=F'].iloc[-1]
        vix_val = raw['vix'].iloc[0] if not raw['vix'].empty else 15.0
        
        cms = (z(ndl, raw['fed']) * weights[0] + 
               z(copper_gold, (y_data['HG=F'] / y_data['GC=F'])) * weights[1] + 
        ...
        return {
            ...
            'copper_gold': round(float(copper_gold), 4),
